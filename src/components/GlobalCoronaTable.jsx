...
export const GlobalCoronaTable = () => {
  const { globalCountries, formatNum } = useContext(CoronaContext);
  ...
  return (
    <section id="global-table" className="mt-10">
      <div className="w-4/5 mx-auto bg-white p-10 rounded-lg shadow-sm max-h-screen overflow-scroll">
        <h3 className="uppercase text-center mb-4 text-lg font-light">global corona virus tracker</h3>
        <table className="table-auto w-full text-left overflow-scroll">
          ...
          <tbody>
            {globalCountries.map((country, index) => {
              const { name, recovered, deaths, confirmed } = country;
              return (
                <tr key={index}>
